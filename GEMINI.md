...
*   **`nlabpy/`**: The main source directory.
    *   **`bin/`**: Contains executable scripts, such as `demux.py` for demultiplexing FASTQ files.
...
*   **`examples/`**: Jupyter notebooks demonstrating usage (e.g., `rnafold.ipynb`, `Coverage_plots.ipynb`).
...
*   **Dependencies:** Managed via `setup.py`. Key requirements include `bokeh`, `pysam`, `plotnine`, and `pandas`.
*   **Testing:** No explicit `tests` directory was found in the root. Verification likely relies on the provided examples and manual testing.
*   **Style:** Follows standard Python conventions.
